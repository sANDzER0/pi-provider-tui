 ...
 */
export async function editHeadersScreen(existing) {
    ...
    for (;;) {
        ...
        if (keys.length > 0) {
            options.push({ value: "edit", label: "Edit value" }, { value: "remove", label: "Remove header" }, {
                value: "done",
        ...
        switch (action) {
            ...
            case "add": {
                const name = await p.text({
                    ...
                    validate: (v) => v && /^[A-Za-z0-9-]+$/.test(v.trim())
                        ? undefined
