...
import { API_TYPES, THINKING_LEVELS, } from "./types.js";
/** APIs pi itself understands (superset of what this TUI edits). */
const PI_KNOWN_APIS = new Set([
    ...API_TYPES,
...
export function examineDoc(doc) {
    const issues = [];
    ...
    for (const id of ids) {
        ...
        if (!pr.api) {
            if (models.length > 0 || pr.baseUrl) {
                issues.push({
                    provider: id,
                    level: "error",
                    message: "missing api — custom providers need one of: " +
                        API_TYPES.join(", "),
                });
            }
        }
        ...
        if (!pr.apiKey) {
            issues.push({
                provider: id,
                level: "info",
                message: "no apiKey — models stay unavailable in /model until auth is set (/login, --api-key, or auth.json)",
            });
        }
