...
export function chunkPages(lines: string[], pageSize = PAGE_SIZE): string[][] {
  const pages: string[][] = [];
 ...
 */
export async function paginatedNote(
  title: string,
  lines: string[],
  pageSize = PAGE_SIZE,
): Promise<void> {
  ...
  for (let page = 0; page < pages.length; page++) {
    const heading =
      pages.length === 1 ? title : `${title} — page ${page + 1}/${pages.length}`;
    ...
    if (handleCancel(more) || !more) return;
  }
}
