...
import { defaultModel, API_TYPES, type ApiType, type ModelsFile } from "./types.js";
...
function asApi(v: string | undefined): ApiType {
  if (!v || !(API_TYPES as string[]).includes(v)) {
    usageError(`--api must be one of: ${API_TYPES.join(", ")}`);
  }
  return v as ApiType;
}

async function loadOrFail(): Promise<ModelsFile> {
  try {
...
async function cmdAdd(argv: string[]): Promise<number> {
  ...
  const csv = values.models
    ...
    .filter(Boolean);
  if (csv && csv.length > 0) {
    modelList = csv.map((mid) =>
      defaultModel({ id: mid, reasoning: guessReasoning(mid) }),
    );
  } else {
    ...
    if (!res.ok) {
      return die(
        `Fetching models failed (${res.error}). Pass --models a,b,c to define them manually.`,
      );
    }
...
async function cmdList(argv: string[]): Promise<number> {
  ...
  for (const id of ids) {
    ...
    const key = pr.apiKey
      ...
      : "(none)";
    info(
      `${id}  |  ${pr.name ?? id}  |  ${pr.api ?? "?"}  |  ${pr.baseUrl}  |  models:${pr.models?.length ?? 0}  |  key:${key}`,
    );
  }
  return 0;
}
...
async function cmdGet(argv: string[]): Promise<number> {
  ...
  if (!provider) return die(`No provider "${id}".`);
  console.log(JSON.stringify(provider, null, 2));
  return 0;
}

async function cmdRemove(argv: string[]): Promise<number> {
  const { values } = parseArgs({
    args: argv,
  ...
  const id = values.id?.trim() || usageError("--id is required");
  const doc = await loadOrFail();
  if (!doc.providers[id]) return die(`No provider "${id}".`);
  if (!values.y) {
  ...
  if (mode === "endpoint") {
    ...
    if (res.ok) {
      if (values.json) {
        console.log(
          JSON.stringify(
            { ok: true, mode, models: res.models.length, skipped: res.skipped },
            null,
            2,
          ),
        );
    ...
    if (values.json) console.log(JSON.stringify({ ok: false, mode, error: res.error }, null, 2));
    else console.error(`FAILED: ${res.error}`);
    return 1;
  }

  const modelId = values.model ?? provider.models?.[0]?.id;
  if (!modelId) return die("Provider has no models; pass --model.");
  const model = provider.models?.find((m) => m.id === modelId);
  if (!model) return die(`No model "${modelId}" in provider "${id}".`);
...
async function cmdUndo(argv: string[]): Promise<number> {
  const { values } = parseArgs({
    args: argv,
