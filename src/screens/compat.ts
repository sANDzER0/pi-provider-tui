...
/**
 * Provider-level compat editor: common pi compatibility fields as tri-state
 * prompts, everything else via a raw JSON editor. The compat object is passed
 * through to pi verbatim; unknown keys are preserved.
 */
...
async function editBool(compat: Record<string, unknown>, f: BoolField): Promise<void> {
  ...
  if (handleCancel(sel)) return;
  if (sel === "unset") delete compat[f.key];
  else compat[f.key] = sel === "true";
}

async function editEnum(compat: Record<string, unknown>, f: EnumField): Promise<void> {
  const cur = compat[f.key];
  const sel = await p.select({
    message: `${f.label} (current: ${describe(cur)})`,
    options: [
    ...
    initialValue:
      typeof cur === "string" && f.values.includes(cur) ? cur : "__unset__",
  });
  if (handleCancel(sel)) return;
  if (sel === "__unset__") delete compat[f.key];
  else compat[f.key] = sel;
}

function renderCompat(compat: Record<string, unknown>): string {
 ...
 */
export async function editCompatScreen(
  existing: Record<string, unknown> | undefined,
): Promise<Record<string, unknown> | null> {
  let compat: Record<string, unknown> = { ...(existing ?? {}) };
  ...
  for (;;) {
    ...
    if (action === "json") {
      const raw = await p.text({
        ...
        validate: (v) => {
          if (!v || !v.trim()) return undefined; // empty = cancel merge
          try {
      ...
      if (handleCancel(raw)) continue;
      const text = String(raw ?? "").trim();
      if (!text) continue;
      compat = { ...compat, ...(JSON.parse(text) as Record<string, unknown>) };
